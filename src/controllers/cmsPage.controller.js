...
const getAllCmsPages = async (req, res) => {
  try {
    ...
    res.status(200).json({success:true, message: "Cms pages fetched successfully", data: pages });
  } catch (error) {
    res.status(500).json({success:false, message: "Server error" });
  }
};
...
const getCmsPageById = async (req, res) => {
  try {
    ...
    res.status(200).json({success:true, message: "Cms page fetched successfully", data: page });
  } catch (error) {
    res.status(500).json({success:false, message: "Server error" });
  }
};
